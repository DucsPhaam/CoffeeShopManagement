...
public class TableController {
    ...
    private void handleSaveTable() {
        ...
        if (isEditMode && currentTable != null) {
            ...
            if (tableDAO.updateTable(currentTable)) {
                showSweetAlert(SweetAlert.AlertType.INFO, "Success", "Table updated successfully.");
                loadTableData();
                handleCloseDialog();
            } else {
                showSweetAlert(SweetAlert.AlertType.WARNING, "Error", "Failed to update table.");
            }
        } else {
            ...
            if (tableDAO.addTable(newTable)) {
                showSweetAlert(SweetAlert.AlertType.INFO, "Success", "Table added successfully.");
                loadTableData();
                handleCloseDialog();
            } else {
                showSweetAlert(SweetAlert.AlertType.WARNING, "Error", "Failed to add table.");
            }
        }
    }
    ...
    private void handleDeleteTable(Table table) {
        ...
        if (confirmAlert.showAndWait().get() == ButtonType.OK) {
            if (tableDAO.deleteTable(table.getId())) {
                showSweetAlert(SweetAlert.AlertType.INFO, "Success", "Table deleted successfully.");
                loadTableData();
            } else {
                showSweetAlert(SweetAlert.AlertType.WARNING, "Error", "Failed to delete table.");
            }
        }
    }
    ...
    private boolean validateInput() {
        if (txtName.getText().trim().isEmpty()) {
            showSweetAlert(SweetAlert.AlertType.WARNING, "Warning", "Please enter table name.");
            txtName.requestFocus();
            return false;
        }

        if (cmbFloor.getValue() == null) {
            showSweetAlert(SweetAlert.AlertType.WARNING, "Warning", "Please select floor.");
        ...
        try {
            int seats = Integer.parseInt(txtSeats.getText().trim());
            if (seats <= 0) {
                showSweetAlert(SweetAlert.AlertType.WARNING, "Warning", "Seats must be greater than 0.");
                txtSeats.requestFocus();
                return false;
            }
        } catch (NumberFormatException e) {
            showSweetAlert(SweetAlert.AlertType.WARNING, "Warning", "Invalid seats format.");
            txtSeats.requestFocus();
            return false;
        }

        if (cmbStatus.getValue() == null) {
            showSweetAlert(SweetAlert.AlertType.WARNING, "Warning", "Please select status.");
